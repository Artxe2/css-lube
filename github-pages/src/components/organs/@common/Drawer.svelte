...
<nav class="z=10000 fixed t=0 l=0 h=100%
	...
	<div class="flex column h=100% w=4
		...
		<div class="flex column"
			...
			<SubDrawer data={subMenu["Documents"]} bind:isOpen={subListOpen.docs} />
		</div>
		<div class="h=.5"></div>
		<div class="flex column">
		...
		<!-- svelte-ignore a11y-no-static-element-interactions -->
		<div class="flex column"
				on:mouseleave={() => subListOpen.ref = false}>
			<NavButton classs=":not(:hover)_.icon>div:nth-of-type(even)/op=0
					:hover_.icon>div:nth-of-type(odd)/op=0"
					on:mouseenter={() => subListOpen.ref = true}
			...
			<SubDrawer data={subMenu["Reference"]} bind:isOpen={subListOpen.ref} />
		</div>
		<div class="fg=1"></div>
		<div class="h=1"></div>
