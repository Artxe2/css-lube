...
${openTag("head", "")}
	...
	${comment("<!-- Automatically generated styles at runtime using MutationObserver by CSS Lubricant -->")}
	${openTag("style ", ` ${name("css-lubricant")}=${string("v0.7.0")}`)}
		. . . . . .
`
...
const code6 = `${html("body", "", `
	${html("div", ` ${name("class")}=${string("flex h=10 ai=center jc=center")}`, `
		${html("div", ` ${name("class")}=${string("flex w=fit-content bg=#fff c=#000 p=.5 br=.5 bs=0_1_1_gray")}`, `
			...
			${html("div", "", `
				${html("h3", ` ${name("class")}=${string(`bold fs=2<f>fv=small-caps</f>`)}`, "CSS Lubricant")} 
...
<CodePage>
	<div class="flex h=10 ai=center jc=center">
		<div class="flex w=fit-content bg=#fff c=#000 p=.5 br=.5 bs=0_1_1_gray">
			...
			<div>
				<h3 class="bold fs=2{index < 0 ? "": " fv=small-caps"}">CSS Lubricant</h3>
