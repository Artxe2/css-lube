...
const code = `${html("div", "", `
	...
	${html("div", ` ${name("class")}=${string(`bg=#${declare(`{${name("bg")}}`)}`)}`, `${html("span", ` ${name("class")}=${string(`c=#${declare(`{${name("bg")}}`)} ft=invert(1)`)}`, `${bracket(`{${name("bg")}}`)}`)}`)}
...
<H2>Performance Test Page - class</H2>
{background.length}rows
<CodePage>{@html code}</CodePage>
<div>
	{#each background as bg}
	<div class="bg=#{bg}"><span class="c=#{bg} ft=invert(1)">{bg}</span></div>
	{/each}
</div>