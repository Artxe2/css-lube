 ...
 *   duration?: number
 *   is_snap?: boolean
 *   onchangeindex?: (event: CustomEvent<number>) => void
 *   snap: (direction: -1 | 0 | 1) => void
 * }}
 */
let {
	classs = "",
...
const handle_mouseup = () => {
	if (is_dragging) {
		...
		if (is_snap) {
			const speed = calculate_snap_move(
				accumulate * duration * 2 / (curr - time_record[0]),
				0
			)
...
const handle_scroll = () => {
	...
	if (index != (observe_index + scroll_index) % container.childElementCount) {
		...
		onchangeindex?.(
			new CustomEvent("changeindex", { detail: index })
		)
	}
}
/**
