...
*/
{
	...
	let compile_special = cname => {
		...
		return escape(cname) + cname.slice(0, i)
			.replace(replace_space_regex, space) + "{" + parse_value(cname.slice(i + 1)) + "}"
	}
	...
	let parse_query = query => {
		...
		query = char == "@"
			? query.slice(1)
			: "media " + query
		return "@" + query.replace(replace_space_regex, space)
	...
	let parse_value = cname => {
		...
		return cname.replace(replace_space_regex, space)
	...
	/** @param {string} substr */
	let replace_calc_oper_inner_callback = substr => substr[0] + space + substr[1] + space + substr[2]
	...
	style_sheet.setAttribute("css-lube", "v1.4.2")
