(() => {
	...
	let build_style_sheet = () => {
		...
		for (let v of media.values()) {
			temp += theme && v[2]
				? v[0].replace(dark_theme_regex, theme == "DARK" ? "color" : 0)
				: v[0]
			temp += v[1] + "}"
		}
	...
	let collect_unique_top_nodes = mr => {
	...
	let compile_style = cname => {
		if (check_has_value_regex.test(cname) && !check_is_open(cname)) {
			if (check_is_raw(cname)) style += get_priority(cname) + "." + compile_raw(cname)
	...
	/** @param {string} substr */
	let replace_calc_oper_callback = substr => substr.replace(replace_calc_oper_inner_regex, replace_calc_oper_inner_callback)
	 ...
	 * @param {string} b
	 */
	let replace_media_callback = (_, a, b) => a + shorthand_for_media_condition.get(b)
	 ...
	 * @param {string} b
	 */
	let replace_shorthand_unit_callback = (_, a, b) => a + b.replace(
	...
	style_sheet.setAttribute("css-lube", "v1.4.0")
	...
	dom.addEventListener(
		...
		() => {
			...
			new MO(mr_list => {
				let size = classes.size
				mr_list.forEach(collect_unique_top_nodes)
