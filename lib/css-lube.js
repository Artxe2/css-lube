...
/* <- custom field
	...
	let childListObserver = new observer(mrList => {
	...
	let classListObserver = new observer(mrList => {
		let size = classes.size
	...
	let searchClassList = target => target.classList.forEach(addClass)

	let styleSheet = dom.createElement("style")
	styleSheet.setAttribute("css-lube", "v1.0.1")
