(() => {
	...
	let replace_calc_oper_inner_regex = /(\b%?)(?! )([+-])(?! )/g
	let replace_calc_oper_regex = /(calc\()(.+?)(?=\))/g

	/** @type {RegExp} */
	// @ts-ignore
	let replace_media_condition_regex = "\\b(?:"
	// @ts-ignore
	for (let key of shorthand_for_media_condition.keys()) replace_media_condition_regex += key + "|"
	replace_media_condition_regex = RE(
		// @ts-ignore
		replace_media_condition_regex.slice(0, replace_media_condition_regex.length - 1) + ")\\b",
		"g"
	)
	 ...
	 * @param {string} b
	 */
	let replace_property_callback = (_, a, b) => a + shorthand_for_properties.get(b)
	/**
	 * @param {string} _
	 * @param {string} a
	 * @param {string} b
	 */
	let replace_value_callback = (_, a, b) => a + shorthand_for_values.get(b)
	/** @param {string} s */
	let replace_shorthand_unit_inner_callback = s => s + default_unit
	/**
	 * @param {string} _
	 * @param {string} a
	 * @param {string} b
	...
	let replace_shorthand_unit_callback = (_, a, b) =>
		a + b.replace(replace_default_unit_inner_regex, replace_shorthand_unit_inner_callback)
	/**
	 * @param {string} _
	 * @param {string} a
	 * @param {string} b
	 ...
	 * @param {string} b
	 */
	let replace_calc_oper_callback = (_, a, b) =>
		a + b.replace(replace_calc_oper_inner_regex, replace_calc_oper_inner_callback)
	/**
	 * @param {string} _
	 * @param {string} a
	 * @param {string} b
	...
	let compile_special = cname => {
		let i = cname.indexOf("/")
		return escape(cname) + cname.slice(0, i).replace(replace_space_regex, " ") + "{" + parse_value(cname.slice(i + 1)) + "}"
	}
	/** @param {string} substr */
	let parse_condition = substr => {
		if (substr.includes("~")) {
			let i = substr.indexOf(":")
			let j = substr.indexOf("~", i + 2)
			let k = j + 1
			while (substr[++k] < "a");
			let name = substr.slice(0, i)
			let unit = substr.slice(k)
			return "(min-" + name + ":" + substr.slice(i + 1, j) + unit +
				") and (max-" + name + ":" + substr.slice(j + 1, k) + unit + ")"
		}
		return "(" + substr + ")"
	}
	/**
	...
	let build_style_sheet = () => {
		...
		for (let v of media.values()) {
			temp += theme && v[2]
				? v[0].replace(dark_theme_regex, `m${theme == "DARK" ? "in" : "ax"}-width:0`)
				: v[0]
			temp += v[1] + "}"
		}
	...
	style_sheet.setAttribute("css-lube-sc", "v1.2.3")
