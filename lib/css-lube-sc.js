...
:root{-webkit-tap-highlight-color:transparent;text-size-adjust:100%;-webkit-text-size-adjust:100%;line-height:1.5;overflow-wrap:break-word;word-break:break-word;tab-size:4}`
	...
	let shorthandUnitRE = /((?:^|;|-)(?:border|bottom|column-rule|end|flex-basis|font-size|gap|grid-template-(?:columns|rows)|height|inset|left|margin(?:-[a-z]+)*|origin|outline|padding(?:-[a-z]+)*|perspective|position|radius|right|shadow|spacing|start|top|width):)(.+?)(?=;|$)/g
	let defaultUnit = "em"
/* <- custom field / field -> */
	let classes = new Set()
...
/* <- field / regex -> */
	...
	keyRE = RE(keyRE.slice(0, keyRE.length - 1) + ")(?=:)", "g")

	let mediaRE = "\\b(?:"
	for (let k of shorthandMedias.keys()) mediaRE += k + "|"
	mediaRE = RE(mediaRE.slice(0, mediaRE.length - 1) + ")\\b", "g")

	let openRE = /\([^)]*$|\\$/
	let priorityRE = /!$/
	let quoteRE = /^(?:[^']*'[^']*'[^']*|[^'])*$/
	let rawRE = /^[-a-z]/
	let shorthandUnitInnerRE = /(?:^| )-?(?:\d*\.)?\d+(?= |$)/g
	let spaceRE = /_/g
	let tildeRE = /~/

	let valueRE = "(^|/|;)("
	for (let k of shorthandValues.keys()) valueRE += k + "|"
	valueRE = RE(valueRE.slice(0, valueRE.length - 1) + ")(?=;|$|!)", "g")
...
/* <- regex / function -> */
	...
	let replace_key_callback = (_, a, b) => a + shorthandKeys.get(b)
	let replace_value_callback = (_, a, b) => a + shorthandValues.get(b)
	let replace_shorthand_unit_inner_callback = s => s + defaultUnit
	let replace_shorthand_unit_callback = (_, a, b) =>
		a + b.replace(shorthandUnitInnerRE, replace_shorthand_unit_inner_callback)
	let replace_calc_inner_callback = (_, a, b) => a + " " + b + " "
	let replace_calc_callback = (_, a, b) => a + b.replace(calcInnerRE, replace_calc_inner_callback)
	...
	let parse_value = t => {
		...
		return t.replace(spaceRE, " ").replace(colonRE, ":")
			...
			.replace(shorthandUnitRE, replace_shorthand_unit_callback)
	...
	let build_style_sheet = () => {
		...
		for (let v of media.values()) {
			temp += theme && v[2]
					? v[0].replace(darkThemeRE, `m${theme == "DARK" ? "in" : "ax"}-width:0`)
					: v[0]
			temp += v[1] + "\n}"
		}
...
/* <- function / runtime -> */
	...
	styleSheet.setAttribute("css-lube-sc", "v1.1.4")
	styleSheet.textContent = "html{display:none}"
