...
*/
{
	...
	style_sheet.setAttribute("css-lube", "v1.8.0")
	body.outerHTML.replace(
		/class="([^"]+)"/g,
		// @ts-ignore: void 0
		(_, class_name) => {
			for (let cname of class_name.match(cname_regex)) classes.add(cname)
		}
	)
	for (let cname of classes) compile_style(cname)
	build_style_sheet()
	body.hidden = false
