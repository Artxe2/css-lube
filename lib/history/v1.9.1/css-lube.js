...
*/
{
	...
	let build_style_sheet = () => {
		...
		style_sheet.textContent = pure_style + (theme ? temp.replace(dark_theme_regex, theme == "DARK" ? "color" : "") : temp)
	}
	/**
	...
	let detect_pending_styles = target => {
		target.classList.forEach(get_cname_callback)
	}
	/**
	 * @param {Element} target
	 * @returns {void}
	...
	let detect_childs_with_classes = target => {
		...
		class_name.match(get_cname_regex)?.forEach(get_cname_callback)
	}
	/**
	 * @param {string} cname
	 * @returns {void}
	 */
	let get_cname_callback = cname => {
	...
	style_sheet.setAttribute("css-lube", "v1.9.1")
	body.hidden = false
	dom.head.append(style_sheet)
	detect_childs_with_classes(body)
	build_style_sheet()
	new MO(mr_list => {
	...
	new MO(mr_list => {
		let size = classes.size
		for (let mr of mr_list) /** @type {NodeListOf<Element>} */(mr.addedNodes)/**/.forEach(collect_unique_top_nodes)
