...
:root{-webkit-tap-highlight-color:transparent;text-size-adjust:100%;-webkit-text-size-adjust:100%;line-height:1.5;overflow-wrap:break-word;word-break:break-word;tab-size:4}`
	...
	let replaceDefaultUnitRegex = /(?<=(?:^|;|-)(?:border|bottom|column-rule|end|flex-basis|font-size|gap|grid-template-(?:columns|rows)|height|inset|left|margin(?:-[a-z]+)*|origin|outline|padding(?:-[a-z]+)*|perspective|position|radius|right|shadow|spacing|start|top|width):(?:[^; ]* )*?-?(?:\d*\.)?\d+)(?= |;|$)/g
	let defaultUnit = "em"
/* <- custom / field -> */
	let classes = new Set()
...
/* <- regex / function -> */
	...
	let replace_property_callback = s => shorthandForProperties.get(s)
	let replace_value_callback = s => shorthandForValues.get(s)
	let replace_calc_oper_callback = s => " " + s + " "
	...
	let parse_value = t => {
		...
		return t.replace(replaceSpaceRegex, " ").replace(replaceColonRegex, ":")
			...
			.replace(replaceDefaultUnitRegex, defaultUnit)
	...
	let build_style_sheet = () => {
		...
		for (let v of media.values()) {
			temp += theme && v[2]
					? v[0].replace(darkThemeRegex, `width${theme == "DARK" ? ">" : "<"}0`)
					: v[0]
			temp += v[1] + "\n}"
		}
...
/* <- function / main -> */
	...
	styleSheet.setAttribute("css-lube", "v1.2.2")
